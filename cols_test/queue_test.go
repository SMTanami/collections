...
func validateQueue[T comparable](expectedOrdering []T, q cols.Queue[T]) (bool, string) {
...
func TestQueue_Type(t *testing.T) {
	...
	t.Run("Queue Should Be Filterable", func(t *testing.T) {
		...
		if !ok {
			t.Error("Queue is not Filterable")
		}
	})
}
